...
WITH RECURSIVE nn (i, arrs, p1, p2) AS (
    ...
    FROM nn
    WHERE nn.i <= (SELECT n FROM input)
)
INSERT INTO largest
SELECT json_array_length(value) FROM nn, json_each(nn.arrs)
WHERE nn.i = (SELECT n + 1 FROM input)
ORDER BY json_array_length(value) DESC LIMIT 3;

SELECT l1.n * l2.n * l3.n
FROM largest AS l1, largest AS l2, largest AS l3
WHERE l1.ROWID = 1 AND l2.ROWID = 2 AND l3.ROWID = 3;
